...
; ADCSRA- ADC control and status register A
	; bit 6 = 1 => means conversion not done
; ADMUX- ADC multiplexer selection register
; ADCL & ADCH - ADC data register
; ADC receives 10 bit number
	; if greater than 900 then no button being pushed
...
check_button:
	...
	wait:	; wait for conversion to complete, then check if bit 6 (i.e. ADSC bit) is set
		...
		clr r23 ; default value of button_is_pressed is 0
		; check if signal is greater than threshold value of 900
		cp DATAL, BOUNDARY_L
		cpc DATAH, BOUNDARY_H
		brsh skip ; higher => no button being pressed
		ldi r23, 0b00000001 ; otherwise load to indicate button is being pressed
		
	skip:	; entering skip means that no button is being pressed
		sts BUTTON_IS_PRESSED, r23 ; store button is pressed val in data memory (initialized in .dseg)
